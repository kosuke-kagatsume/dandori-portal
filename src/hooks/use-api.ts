...
// ユーザー一覧取得
export function useUsers(tenantId: string = 'tenant-demo-001') {
...
// 勤怠データ取得
export function useAttendance(tenantId: string = 'tenant-demo-001', params?: {
  userId?: string;
  startDate?: string;
  endDate?: string;
}) {
...
// SaaSサービス一覧取得
export function useSaaSServices(tenantId: string = 'tenant-demo-001', includeDetails: boolean = false) {
...
// SaaSプラン一覧取得
export function useSaaSPlans(tenantId: string = 'tenant-demo-001', serviceId?: string) {
  const searchParams = new URLSearchParams({ tenantId });
...
// SaaSライセンス割り当て取得
export function useSaaSAssignments(tenantId: string = 'tenant-demo-001', serviceId?: string) {
  const searchParams = new URLSearchParams({ tenantId });
  if (serviceId) searchParams.set('serviceId', serviceId);
...
// 車両一覧取得
export function useVehicles(tenantId: string = 'tenant-demo-001') {
...
// PC資産一覧取得
export function usePCAssets(tenantId: string = 'tenant-demo-001') {
...
// メンテナンス記録取得
export function useMaintenanceRecords(tenantId: string = 'tenant-demo-001', vehicleId?: string) {
  const searchParams = new URLSearchParams({ tenantId });
...
// 業者一覧取得
export function useVendors(tenantId: string = 'tenant-demo-001') {
...
// 法令更新一覧取得
export function useLegalUpdates(tenantId: string = 'tenant-demo-001') {
...
// マスターデータ（部署）取得
export function useDepartments(tenantId: string = 'tenant-demo-001') {
...
// マスターデータ（役職）取得
export function usePositions(tenantId: string = 'tenant-demo-001') {
...
// マスターデータ（雇用形態）取得
export function useEmploymentTypes(tenantId: string = 'tenant-demo-001') {
...
// ワークフロー一覧取得
export function useWorkflows(tenantId: string = 'tenant-demo-001', status?: string) {
  const searchParams = new URLSearchParams({ tenantId });
...
// 承認フロー一覧取得
export function useApprovalFlows(tenantId: string = 'tenant-demo-001') {
