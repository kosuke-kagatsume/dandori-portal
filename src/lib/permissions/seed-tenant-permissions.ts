/**
 * テナント作成時に権限データを自動投入するユーティリティ
 */

import { PrismaClient } from '@prisma/client';

// Prismaトランザクションクライアントの型（$transaction内で渡されるクライアント）
...
/**
 * 指定テナントに権限データ（ロール・権限マッピング）を投入する
 *
 * @param prisma - Prismaクライアント（トランザクション内で使用可能）
...
export async function seedTenantPermissions(
  ...
  console.log(`[seedTenantPermissions] テナント ${tenantId} の権限データを投入中...`);

  // 1. 権限マスタの作成（テナント非依存・共有）
  ...
  for (const roleRec of roleRecords) {
    ...
    for (const code of permCodes) {
      ...
      if (!perm) {
        console.warn(`[seedTenantPermissions] 警告: 権限 "${code}" がマスタに存在しません`);
        continue;
      }
  ...
  console.log(`[seedTenantPermissions] テナント ${tenantId} の権限データ投入完了`);
}
