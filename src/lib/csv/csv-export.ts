...
export const exportSubmissionRateToCSV = (
  ...
  try {
    if (!employees || employees.length === 0) {
      return { success: false, error: 'エクスポートするデータがありません', recordCount: 0 };
    }
    ...
    const rows = employees.map((e) => [
      ...
      e.submissionRate,
      e.lastSubmittedDate
        ? new Date(e.lastSubmittedDate).toLocaleDateString('ja-JP')
        : '',
    ]);

    const csvString = generateCSVString(headers, rows);
    const defaultFilename = `daily_report_submission_rate_${getCurrentDate()}.csv`;
    downloadCSV(csvString, filename || defaultFilename);

    exportAudit.csv('日報提出率データ', employees.length);
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : '提出率CSVの出力に失敗しました',
      recordCount: 0,
    };
  }
};
...
export const exportDailyReportListToCSV = (
  ...
  try {
    ...
    const csvString = generateCSVString(headers, rows);
    const defaultFilename = `daily_report_list_${getCurrentDate()}.csv`;
    downloadCSV(csvString, filename || defaultFilename);

    exportAudit.csv('日報一覧データ', employees.length);
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : '日報一覧CSVの出力に失敗しました',
      recordCount: 0,
    };
  }
};
...
export const exportFieldAggregationToCSV = (
  ...
  try {
    if (!aggregation || aggregation.fields.length === 0) {
      return { success: false, error: 'エクスポートするデータがありません', recordCount: 0 };
    }
    ...
    const rows = aggregation.fields.map((f) => {
      ...
      return [
        aggregation.templateName,
        aggregation.reportCount,
        f.fieldId,
        f.label,
        f.fieldType,
        s.type,
    ...
    const csvString = generateCSVString(headers, rows);
    const defaultFilename = `daily_report_field_aggregation_${getCurrentDate()}.csv`;
    downloadCSV(csvString, filename || defaultFilename);

    exportAudit.csv('日報フィールド集計データ', aggregation.fields.length);

    return { success: true, recordCount: aggregation.fields.length };
  } catch (error) {
    console.error('Failed to export field aggregation CSV:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'フィールド集計CSVの出力に失敗しました',
      recordCount: 0,
    };
  }
};
