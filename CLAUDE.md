...
**2026-02-26追加**: 認証・テナント問題の見落とし事故から、以下のルールを追加

### バグ修正前の必須確認

1. **コードフロー全体をトレース** - 症状だけ見て修正しない
2. **関連ファイルを全てリストアップ** - 認証なら middleware.ts, 全auth API, api-helpers.ts
3. **修正の副作用を検証** - 「この変更で他が壊れないか？」

### 認証関連の注意点

| 確認項目       | ファイル                                                          |
| -------------- | ----------------------------------------------------------------- |
| パブリックパス | `middleware.ts` の `publicPaths` 配列                             |
| Cookie名の一致 | `access_token`, `dw_admin_token` 等が全ファイルで統一されているか |
| Cookie設定     | `path: '/'`, `sameSite: 'lax'` が設定されているか                 |

### テナント関連の注意点

| 設定元          | 役割                                               |
| --------------- | -------------------------------------------------- |
| middleware.ts   | サブドメインから `x-tenant-id` を設定（正）        |
| login API       | `x-tenant-id` を設定しない（ミドルウェアに任せる） |
| proxy-login API | `x-tenant-id` を設定する（メインドメイン用）       |

詳細: `docs/BUG_FIX_CHECKLIST.md`
