...
**⚠️ 超重要: デプロイはAWS Amplifyを使用（Vercelではない！）**

| 環境 | ブランチ | URL | 用途 |
|------|---------|-----|------|
| **本番** | `main` | https://dandori-portal.com | 本番環境 |
| **開発** | `develop` | https://dev.dandori-portal.com | 開発・検証環境 |
...
Amplifyが自動でビルド・デプロイを実行します（GitHubと連携済み）。

### AWS リソース

| サービス | リソース名 | 用途 |
|---------|-----------|------|
| **Amplify** | dandori-portal | フロントエンドホスティング |
| **RDS** | PostgreSQL | データベース |
| **S3** | dandori-portal-storage | ファイルストレージ |
